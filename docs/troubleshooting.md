...
Common issues when working with the Glaucus YAML library, organized by symptom.
...
1. **Indentation mismatch** — Glaucus is strict about indentation. Mixed tabs and spaces will fail.
2. **Duplicate keys** — Glaucus defaults to `Strictness::Strict`, which rejects duplicate keys. Use `Strictness::Lenient` to allow them.
...
let mut scanner = glaucus_core::scanner::Scanner::new(input);
...
3. **EOF handling**: Glaucus treats EOF as an implicit line break (YAML spec)
...
**Expected behavior.** Glaucus normalizes output:
...
```bash
# Read the test data
ls glaucus-yaml-test-suite/data/XXXX/
