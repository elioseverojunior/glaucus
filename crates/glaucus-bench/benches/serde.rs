...
fn serde_deserialize_benchmark(c: &mut Criterion) {
    ...
    for (name, yaml, json) in [
        ...
        group.bench_with_input(BenchmarkId::new("glaucus", name), &yaml, |b, input| {
            b.iter(|| glaucus::from_str::<glaucus::Value>(black_box(input)).unwrap());
        });
...
fn serde_serialize_benchmark(c: &mut Criterion) {
    ...
    let glaucus_small: glaucus::Value = glaucus::from_str(SMALL_POD).unwrap();
    let glaucus_medium: glaucus::Value = glaucus::from_str(MEDIUM_HELM).unwrap();
    let glaucus_large: glaucus::Value = glaucus::from_str(&large_yaml).unwrap();
    ...
    for (name, yaml_len, glaucus_val, json_val, noyalib_val, ng_val, saphyr_val) in [
        (
            "small",
            SMALL_POD.len(),
            &glaucus_small,
        ...
        group.bench_with_input(BenchmarkId::new("glaucus", name), glaucus_val, |b, val| {
            b.iter(|| glaucus::to_string(black_box(val)).unwrap());
        });
