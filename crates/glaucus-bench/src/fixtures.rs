...
//! for throughput benchmarking across the Glaucus pipeline.

use std::fmt::Write;

// ─── Small: K8s Pod Spec (218 bytes) ─────────────────────────────────
...
// ─── Medium: Helm Values (3364 bytes, ~3.3KB) ───────────────────────
...
/// and nested mapping fields — roughly 156 bytes per entry.
/// `generate_large(800)` produces 124,579 bytes (~121.7KB).
// Every `write!` here targets a `String`, whose `fmt::Write` is infallible, and
// the `as f64` only feeds a benchmark fixture's decimal field where a few ulps
// of drift past 2^53 would not change what is being measured.
...
// Same fixture data as `generate_large`, rendered as JSON; see the note there.
#[allow(clippy::unwrap_used, clippy::cast_precision_loss)]
#[must_use]
...
mod tests {
    ...
    #[test]
    fn small_pod_parses_with_glaucus() {
        glaucus_ast::composer::compose_all(SMALL_POD).unwrap();
    }
    ...
    #[test]
    fn medium_helm_parses_with_glaucus() {
        glaucus_ast::composer::compose_all(MEDIUM_HELM).unwrap();
    }
    ...
    #[test]
    fn large_parses_with_glaucus() {
        let large = generate_large(100);
        glaucus_ast::composer::compose_all(&large).unwrap();
    }
    ...
    fn large_generator_produces_expected_size() {
        ...
        // ~156 bytes per entry + 9 bytes header = 124,579 bytes (~121.7KB).
        // The bounds stay deliberately wide: they guard against the generator
        // breaking outright, not against the exact byte count drifting.
