...
Criterion benchmark suite for the Glaucus YAML pipeline.

> **Dev-only crate.** `publish = false` — not released to crates.io. It exists purely to measure Glaucus and is never part of the shipped dependency graph.
...
```sh
cargo bench -p glaucus-bench
```

Run a single benchmark target (`--bench <name>`):

```sh
cargo bench -p glaucus-bench --bench scanner
cargo bench -p glaucus-bench --bench parser
cargo bench -p glaucus-bench --bench composer
cargo bench -p glaucus-bench --bench emitter
cargo bench -p glaucus-bench --bench serde
cargo bench -p glaucus-bench --bench end_to_end
```
...
| ---------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `scanner`        | `scanner`                                                                         | Tokenizing `&str` → `Token` (`Scanner::next_token` loop). Glaucus only — no competitor token API.   |
...
Benched against the YAML/serde ecosystem. All are **dev-dependencies of `glaucus-bench` only**, and each carries a `deny.toml` `skip-tree` exemption so its transitive tree does not trip the multiple-versions gate — the shipped `glaucus-*` graph stays competitor-free.
