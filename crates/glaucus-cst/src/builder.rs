...
impl GreenNodeBuilder {
    ...
    /// # Panics
    ///
    /// Panics if no node is open — `start_node` must precede any `token`. This
    /// is a builder-protocol violation by the caller, not a property of the
    /// text being built.
    ...
    /// # Panics
    ///
    /// Panics if there is no open node to close, i.e. `finish_node` was called
    ...
    /// # Panics
    ///
    /// Panics if any node is still open, or if no node was ever completed, so
    /// there is no root to return. Both mean the begin/end calls were unbalanced.
    #[must_use]
    #[allow(clippy::expect_used)]
