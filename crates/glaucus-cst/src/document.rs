...
//! it follows (e.g. `"k" :`, reproduced on `glaucus-yaml-test-suite/data/26DV`).
...
//! (reproduced on `glaucus-yaml-test-suite/data/26DV`). The plan explicitly
...
use glaucus_core::parser::{Event, EventKind, Parser};
use glaucus_core::scanner::Scanner;
use glaucus_core::scanner::token::{Token, TokenKind};
...
impl std::fmt::Display for SetError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::NotAMapping => write!(f, "root is not a block mapping"),
        }
    }
}
...
where
    I: Iterator<Item = glaucus_core::error::Result<Event<'a>>>,
{
    loop {
        match events.next() {
...
const fn leaf_kind(kind: &TokenKind<'_>) -> SyntaxKind {
    match kind {
...
impl Document {
    ...
    ///
    /// Resolution runs over the [`glaucus_core::parser::Parser`] event stream of
    ...
    loop {
        match events.peek() {
            ...
            Some(Ok(_)) => {
                ...
                let key = read_key(events)?;
                let matched = found.is_none() && key == Key::Scalar(target.to_string());
                if matched {
...
/// What reading one mapping key produced.
#[derive(Debug, PartialEq, Eq)]
enum Key {
    /// A scalar key, with its text.
    Scalar(String),
    /// A non-scalar key (e.g. a complex `? [a]`) whose subtree was consumed.
    Skipped,
}

/// Reads one key node, or `None` if the stream errored or ended.
fn read_key<'a, I>(events: &mut Events<'a, I>) -> Option<Key>
where
    I: Iterator<Item = glaucus_core::error::Result<Event<'a>>>,
{
    let Some(Ok(ev)) = events.peek() else {
        return None;
    };
    if let EventKind::Scalar { value, .. } = &ev.kind {
        let text = value.to_string();
        events.next();
        return Some(Key::Scalar(text));
    }
...
const fn preserves_trailing_ws(text: &str) -> bool {
...
fn collect_tape(input: &str) -> (Vec<Token<'_>>, Option<usize>) {
    // Annotated because the error path reads `tape` before the push that would
    // otherwise pin the element type.
    let mut tape: Vec<Token<'_>> = Vec::new();
...
fn build(input: &str) -> Rc<GreenNode> {
    ...
    let tail_from = scan_stop.map_or(covered, |stop| covered.max(stop).min(input.len()));
...
mod tests {
    ...
    fn insert_appends_top_level_key_preserving_comments() {
        let mut doc = Document::parse("name: glaucus  # the name\n");
        doc.insert("version", "1").unwrap();
        assert_eq!(doc.to_string(), "name: glaucus  # the name\nversion: 1\n");
    ...
    fn ev(kind: EventKind<'static>) -> Event<'static> {
        Event {
            kind,
    ...
    fn resolve_rejects_non_node_start_event() {
        ...
        let events: Vec<CoreResult<_>> = vec![Ok(ev(EventKind::MappingEnd))];
    ...
    fn skip_node_rejects_non_node_start_event() {
        ...
        let events: Vec<CoreResult<_>> = vec![Ok(ev(EventKind::SequenceEnd))];
        let mut it = events.into_iter().peekable();
    ...
    fn corpus_lossless() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../glaucus-yaml-test-suite/data");
        if !dir.exists() {
