...
impl SyntaxElement {
    ...
    pub fn text_range(&self) -> (usize, usize) {
        match self {
            ...
            Self::Token(t, off) => (*off, off + t.text_len()),
        }
    }
    ...
    pub fn text(&self) -> String {
        match self {
            Self::Node(n) => n.text(),
            Self::Token(t, _) => t.text().to_string(),
        }
    }
...
impl SyntaxNode {
    ...
    #[must_use]
    pub const fn new_root(green: Rc<GreenNode>) -> Self {
        Self { green, offset: 0 }
    }
    ...
    pub fn children_with_tokens(&self) -> impl Iterator<Item = SyntaxElement> + '_ {
        ...
        self.green.children().iter().map(move |c| {
            ...
            cur += c.text_len();
            match c {
                GreenChild::Node(n) => SyntaxElement::Node(Self {
                    green: n.clone(),
                    offset: start,
                }),
