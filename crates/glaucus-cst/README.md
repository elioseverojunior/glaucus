...
**Glaucus lossless concrete syntax tree (CST) for comment-preserving edits.**

`glaucus-cst` is a lossless concrete syntax tree for YAML — a Roslyn /
...
`glaucus-ast`. The AST is lossy by design — it captures *meaning* (mappings,
...
editing substrate behind the `glaucus` CLI's `fmt` (safe formatter), `lsp`
(language server), and `mcp` (edit tools) commands, all behind the `cli`
feature.

It depends only on `glaucus-core` (the shared scanner/parser front end) and is
`#![forbid(unsafe_code)]`.
...
graph TD
    SRC["&str source"]
    SCAN["glaucus-core<br/>Scanner (with trivia tape)"]
...
Losslessness holds *by construction*. `Document::parse` drains `glaucus-core`'s
...
```rust
use glaucus_cst::{Document, SyntaxElement};
...
```rust
use glaucus_cst::{GreenNodeBuilder, SyntaxKind, SyntaxNode};
...
```bash
cargo test -p glaucus-cst
```

**Run only integration tests:**

```bash
cargo test -p glaucus-cst --test integration
```

**With output:**

```bash
cargo test -p glaucus-cst --test integration -- --nocapture
```
