...
fn parse_logic(text: &str) -> Result<(), String> {
    glaucus::from_str_node(text).map(|_| ()).map_err(|e| {
...
fn format_logic(text: &str) -> Result<String, String> {
    glaucus::from_str_node(text).map_err(|e| format!("parse error: {e}"))?;
    Ok(glaucus::cst::Document::parse(text).reformatted())
}

fn validate_logic(text: &str, schema: &str) -> Result<String, String> {
    let schema_node =
        glaucus::from_str_node(schema).map_err(|e| format!("schema parse error: {e}"))?;
    let sc = glaucus::schema::Schema::from_node(&schema_node);
    let data_node = glaucus::from_str_node(text).map_err(|e| format!("parse error: {e}"))?;
    let diags: Vec<Value> = match glaucus::schema::validate(&data_node, &sc) {
        Ok(()) => Vec::new(),
        Err(errs) => errs
            .into_iter()
            .map(|e| {
...
fn edit_logic(text: &str, path: &str, value: &str) -> Result<String, String> {
    let mut doc = glaucus::cst::Document::parse(text);
    match doc.set(path, value) {
        Ok(()) => Ok(doc.to_string()),
        Err(glaucus::cst::SetError::PathNotFound) if !path.contains('.') => {
...
/// # Errors
///
/// Returns the parse error message if `text` is not valid YAML.
#[wasm_bindgen]
...
/// # Errors
///
/// Returns a parse error message if either `text` or `schema` is invalid.
#[wasm_bindgen]
...
/// # Errors
///
/// Returns an error message if `text` is not valid YAML, or if `path` cannot
...
/// Returns the `glaucus-wasm` package version.
#[wasm_bindgen]
#[must_use]
