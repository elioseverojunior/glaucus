...
**WebAssembly (wasm-bindgen) bindings for Glaucus: parse/format/validate/edit YAML in the browser or Node.**

`glaucus-wasm` exposes Glaucus's safety-first YAML 1.2.2 engine to JavaScript. Compile it to a WebAssembly module and call `yaml_parse`, `yaml_format`, `yaml_validate`, and `yaml_edit` directly from the browser (ESM) or Node — no native YAML dependency, the same engine that passes 100% of the official YAML test suite.
...
wasm-pack build glaucus-wasm --target web

# Node.js (CommonJS)
wasm-pack build glaucus-wasm --target nodejs

# Bundler-pipeline (webpack/rollup)
wasm-pack build glaucus-wasm --target bundler
```
...
```sh
cargo build -p glaucus-wasm --target wasm32-unknown-unknown --release
wasm-bindgen target/wasm32-unknown-unknown/release/glaucus_wasm.wasm \
...
| `version`      | _(none)_                                     | The `glaucus-wasm` package version `string`                                                 |
...
```js
const { yaml_validate } = require("./pkg/glaucus_wasm.js");
...
`glaucus-wasm` keeps every `#[wasm_bindgen]` export a **one-line delegate** to a pure `*_logic` function. The logic functions contain no `unsafe` and no wasm dependency, so they are unit-tested on the host. The wasm layer only marshals strings across the ABI boundary.

```mermaid
graph LR
    JS["JS caller<br/><i>browser / Node</i>"]
    subgraph WASM["glaucus-wasm (cdylib)"]
        direction TB
        EXPORT["#[wasm_bindgen] export<br/><i>thin one-line delegate</i>"]
        LOGIC["pure *_logic fn<br/><i>no unsafe · host-tested</i>"]
        EXPORT --> LOGIC
    end
    ENGINE["glaucus engine<br/><i>ast · cst · schema</i>"]
...
| --------------- | ---------------- | ---------------------------------------------------- |
| `yaml_parse`    | `parse_logic`    | `glaucus::from_str_node`                               |
| `yaml_format`   | `format_logic`   | `glaucus::from_str_node` + `glaucus::cst::Document`      |
| `yaml_validate` | `validate_logic` | `glaucus::schema::{Schema, validate}` + `serde_json`   |
| `yaml_edit`     | `edit_logic`     | `glaucus::cst::Document` (`set` / `insert`)            |

## Safety

`glaucus-wasm` is the **only** crate in the Glaucus workspace that does **not** set `#![forbid(unsafe_code)]`.

The reason is mechanical: the `#[wasm_bindgen]` macro expands to FFI/ABI glue containing `unsafe` blocks inside this crate, which cannot compile under `forbid`. This is a **deliberate, crate-local, approved exception** for the WebAssembly satellite. `glaucus-core` remains `#![forbid(unsafe_code)]` with no exceptions.
