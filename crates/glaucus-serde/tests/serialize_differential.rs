...
//! ([`glaucus_serde::to_string`]) must always produce byte-identical output to
//! the Node-path oracle ([`glaucus_serde::ser::to_node`] +
...
//!
//! Note: the plan describes `glaucus::Value` / `glaucus::from_str`, but `glaucus-serde`
//! cannot depend on the `glaucus` facade (that would be circular — `glaucus`
//! depends on `glaucus-serde`). The crate-local equivalents `glaucus_serde::Value`
//! and `glaucus_serde::from_str::<Value>` are used instead; they are the exact
//! types the facade re-exports.

// `.clippy.toml`'s allow-expect-in-tests / allow-unwrap-in-tests only cover
// `#[cfg(test)]` modules. An integration test is its own crate with no such
// cfg, so the workspace lints apply here and have to be answered directly.
...
fn assert_same<T: Serialize>(v: &T) {
    let streamed = glaucus_serde::to_string(v).expect("stream");
    let via_node = {
        let node = glaucus_serde::ser::to_node(v).expect("to_node");
        glaucus_ast::emitter::emit_to_string(&node, &glaucus_ast::emitter::EmitterConfig::default())
    };
...
fn assert_doc_same(yaml: &str) {
    let v: Value = glaucus_serde::from_str(yaml).expect("parse YAML into Value");
    assert_same(&v);
}
