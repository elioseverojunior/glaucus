...
**Hand-rolled, Span-anchored JSON-Schema validation over the Glaucus `Node` tree.**

`glaucus-schema` validates a YAML document — represented as a Glaucus `Node` tree —
...
the fixes are applied over the lossless CST (`glaucus-cst`), not the AST.

It depends on `glaucus-core` (`Span`), `glaucus-ast` (the `Node` tree it validates),
and `glaucus-cst` (the lossless `Document` it edits). The crate is
`#![forbid(unsafe_code)]` with `#![deny(missing_docs)]`.

This crate powers the `glaucus validate` CLI command (`--fix` autofix) and the
`yaml_validate` tool exposed by the `glaucus mcp` command.
...
graph TD
    ...
    SRC -->|glaucus-ast compose| NODE["Node tree"]
    ...
    SRC -->|glaucus-cst parse| DOC["cst::Document<br/><i>lossless</i>"]
...
use glaucus_schema::{Schema, validate};
...
use glaucus_schema::{Schema, apply_defaults, coerce_to_schema};
