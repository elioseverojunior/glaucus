...
`glaucus-ast` is the representation-tree layer of the [Glaucus](../../README.md) YAML
1.2.2 toolkit. It sits directly above `glaucus-core`: the core front-end turns
text into a stream of parser **events**, and `glaucus-ast` turns those events into
an in-memory `Node` tree (the **composer**) and turns a `Node` tree back into
YAML text (the **emitter**).

This crate is the foundation that higher layers build on — `glaucus-serde` and the
tooling crates consume the `Node` tree rather than raw events. Its only
dependency is `glaucus-core`; it pulls in no `serde` and no third-party code, and
...
```mermaid
graph LR
    EVENTS["Event stream<br/><i>from glaucus-core</i>"]
...
```rust
use glaucus_ast::compose_all;

let docs = compose_all("name: glaucus\nport: 8080\n").unwrap();
assert_eq!(docs.len(), 1);

let entries = docs[0].as_mapping().unwrap();
assert_eq!(entries[0].0.as_str(), Some("name"));
assert_eq!(entries[0].1.as_str(), Some("glaucus"));
```
...
```rust
use glaucus_ast::{compose_all, emit_to_string, EmitterConfig};
...
The `Composer` drives `glaucus-core`'s parser and folds its event stream into a
...
- **Resource limits.** Node count, scalar/key length, and alias-expansion limits
  from `glaucus-core`'s `ResourceLimits` are enforced as the tree is built,
