...
/// Push-based YAML emitter sink that owns all YAML layout.
///
/// Indentation, block/flow rendering, the deferred-separator state machine,
/// empty-collection `[]`/`{}` and punctuation all live here, behind an
/// imperative API over an explicit `Frame` stack — no recursion, no `Node`.
/// Two drivers feed it: the `Node` walk in `emit_to_string` and the streaming
/// serde `Serializer` in `glaucus-serde`.
///
...
impl<'c, W: fmt::Write> Emitter<'c, W> {
    ...
    /// Helper to resolve a deferred map-value separator on a specific frame.
    /// Uses `TrackingWriter<W>` to match your struct's exact internal writer type.
    fn resolve_frame_sep(
        w: &mut TrackingWriter<W>,
        frame: &mut Frame,
        child_empty: bool,
    ) -> fmt::Result {
        if frame.pending == Pending::MapValue {
    ...
    fn take_map_value_sep(&mut self, child_empty: bool) -> fmt::Result {
        if let Some(parent) = self.stack.last_mut() {
            Self::resolve_frame_sep(&mut self.w, parent, child_empty)?;
        }
        Ok(())
    }
    ...
    /// # Errors
    ///
    /// Propagates any error returned by the underlying [`fmt::Write`].
    pub fn scalar(&mut self, value: &str, style: ScalarStyle) -> fmt::Result {
        ...
        match action {
            ...
            // Key and flow item both emit bare text with no trailing newline:
            // a key is followed by the value separator, a flow item by the
            // parent's `", "`.
            Action::Key | Action::FlowItem => {
                self.write_pending_scalar_tag()?;
    ...
    /// # Panics
    ///
    /// Panics if called outside a sequence. The sink is driven by `drive_node`,
    /// which always pairs the begin/end calls, so reaching this means the
    /// caller broke that protocol rather than that the input was bad.
    ...
    /// # Panics
    ///
    /// Panics if called without a matching `begin_seq`. The sink is driven by `drive_node`,
    /// which always pairs the begin/end calls, so reaching this means the
    /// caller broke that protocol rather than that the input was bad.
    #[allow(clippy::expect_used)]
    ...
    /// # Panics
    ///
    /// Panics if called outside a mapping. The sink is driven by `drive_node`,
    /// which always pairs the begin/end calls, so reaching this means the
    /// caller broke that protocol rather than that the input was bad.
    #[allow(clippy::expect_used)]
    ...
    /// # Panics
    ///
    /// Panics if called without a matching `begin_map`. The sink is driven by `drive_node`,
    /// which always pairs the begin/end calls, so reaching this means the
    /// caller broke that protocol rather than that the input was bad.
    #[allow(clippy::expect_used)]
    ...
    fn finish_empty_collection_line(&mut self) -> fmt::Result {
        match self.stack.last() {
            ...
            // Top level, or a block parent: this line ends here.
            _ => writeln!(self.w),
        }
    }
    ...
    fn finish_flow_collection_line(&mut self) -> fmt::Result {
        match self.stack.last() {
            // A nested flow parent's `", "` handles separation.
            Some(f) if f.style == CollectionStyle::Flow => Ok(()),
            // Top level, or a block parent (map value / seq element).
            _ => writeln!(self.w),
        }
    }
...
mod tests {
    ...
    use glaucus_core::types::ScalarStyle;
