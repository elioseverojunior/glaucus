...
";
    let pod: K8sPod = glaucus::from_str(yaml).unwrap();
    ...
    // Round-trip
    let yaml_out = glaucus::to_string(&pod).unwrap();
    let roundtripped: K8sPod = glaucus::from_str(&yaml_out).unwrap();
...
"#;
    let compose: DockerCompose = glaucus::from_str(yaml).unwrap();
    ...
    // Round-trip
    let yaml_out = glaucus::to_string(&compose).unwrap();
    let roundtripped: DockerCompose = glaucus::from_str(&yaml_out).unwrap();
...
";
    let wf: GHWorkflow = glaucus::from_str(yaml).unwrap();
...
fn serde_rename_all() {
    ...
    let config: ApiConfig = glaucus::from_str(yaml).unwrap();
    assert_eq!(config.api_key, "sk-123");
    assert_eq!(config.max_retries, 3);

    let yaml_out = glaucus::to_string(&config).unwrap();
    let roundtripped: ApiConfig = glaucus::from_str(&yaml_out).unwrap();
...
fn serde_default() {
    ...
    let config: Config = glaucus::from_str("name: app").unwrap();
...
fn serde_skip() {
    ...
    let yaml = glaucus::to_string(&Secret {
        name: "admin".into(),
...
fn serde_enum_externally_tagged() {
    ...
    // Unit variant
    let yaml = glaucus::to_string(&Action::Wait).unwrap();
    let a: Action = glaucus::from_str(&yaml).unwrap();
    assert_eq!(a, Action::Wait);

    // Newtype variant
    let yaml = glaucus::to_string(&Action::Type("hello".into())).unwrap();
    let a: Action = glaucus::from_str(&yaml).unwrap();
    assert_eq!(a, Action::Type("hello".into()));

    // Struct variant
    let yaml = glaucus::to_string(&Action::Click { x: 10, y: 20 }).unwrap();
    let a: Action = glaucus::from_str(&yaml).unwrap();
...
fn serde_flatten() {
    ...
    let req: Request = glaucus::from_str(yaml).unwrap();
...
fn value_as_untyped_yaml() {
    let value: glaucus::Value =
        glaucus::from_str("name: glaucus\nversion: 1\ntags:\n  - yaml\n  - rust").unwrap();
...
fn empty_collections() {
    ...
    let data: Data = glaucus::from_str("items: []\ntags: {}").unwrap();
    assert!(data.items.is_empty());
    assert!(data.tags.is_empty());

    let yaml = glaucus::to_string(&data).unwrap();
    let roundtripped: Data = glaucus::from_str(&yaml).unwrap();
...
fn nested_collections() {
    ...
    let yaml = "matrix:\n  - - 1\n    - 2\n  - - 3\n    - 4";
    let grid: Grid = glaucus::from_str(yaml).unwrap();
...
fn special_float_values() {
    ...
    let f: Floats = glaucus::from_str(yaml).unwrap();
    assert!(f.pos_inf.is_infinite() && f.pos_inf > 0.0);
    assert!(f.neg_inf.is_infinite() && f.neg_inf < 0.0);
    assert!(f.nan.is_nan());
    // Compare bit patterns: the assertion is that `0.0` round-tripped exactly,
    // including its sign, not that it landed within some tolerance.
    assert_eq!(f.zero.to_bits(), 0.0_f64.to_bits());
}
...
fn quoted_booleans_are_strings() {
    ...
    let c: Config = glaucus::from_str("flag: 'true'").unwrap();
...
fn multiline_string_value() {
    ...
    let yaml = "content: |\n  line1\n  line2\n  line3\n";
    let doc: Doc = glaucus::from_str(yaml).unwrap();
...
fn borrowed_value_via_facade() {
    let input = String::from("name: glaucus\n");
    let bv = glaucus::BorrowedValue::parse(&input).unwrap();
...
fn merge_keys_resolve_when_enabled() {
    ...
    let node = glaucus_ast::composer::Composer::with_config(
        "base: &b {a: 1}\nderived:\n  <<: *b\n  b: 2\n",
        cfg,
    )
    .next()
...
fn lenient_mode_allows_duplicate_keys() {
    ...
    let config = glaucus::error::ParserConfig {
        strictness: glaucus::error::Strictness::Lenient,
        ..Default::default()
    };
    let result: HashMap<String, i32> = glaucus::from_str_with("a: 1\nb: 2\na: 3", config).unwrap();
...
fn strict_mode_rejects_duplicate_keys() {
    let config = glaucus::error::ParserConfig {
        strictness: glaucus::error::Strictness::Strict,
        ..Default::default()
    };
...
fn deny_anchors_policy_end_to_end() {
    use glaucus::error::{ParserConfig, ParserPolicies};
    let cfg = ParserConfig {
    ...
    let res = glaucus_ast::composer::Composer::with_config("x: &a 1\ny: *a\n", cfg)
        .next()
        .unwrap();
    assert!(res.is_err());
}
...
fn max_scalar_length_policy_end_to_end() {
    ...
    assert!(
        glaucus_ast::composer::Composer::with_config("k: toolong\n", cfg)
            .next()
            .unwrap()
            .is_err()
    );
...
fn deny_tags_policy_end_to_end() {
    ...
    assert!(
        glaucus_ast::composer::Composer::with_config("k: !!str v\n", cfg)
            .next()
            .unwrap()
            .is_err()
    );
...
fn yaml_1_1_compat_end_to_end() {
    ...
    assert!(glaucus::from_str_with::<bool>("on", cfg.clone()).unwrap());
    assert!(!glaucus::from_str_with::<bool>("off", cfg).unwrap());
    // Under default YAML 1.2, "on" is just a plain string
    assert_eq!(glaucus::from_str::<String>("on").unwrap(), "on");
}
...
fn output_styling_via_facade() {
    let yaml = glaucus::to_string(&glaucus::FlowSeq(vec![1, 2, 3])).unwrap();
    assert!(
        yaml.contains('[') && yaml.trim_end().ends_with(']'),
        "got: {yaml:?}"
    );
    let lit = glaucus::to_string(&glaucus::LitStr("a\nb\n")).unwrap();
    assert!(lit.contains('|'), "got: {lit:?}");
    let fold = glaucus::to_string(&glaucus::FoldStr("x y\n")).unwrap();
...
fn coerce_to_schema_via_facade_preserves_comments() {
    let sc_node = glaucus::from_str_node("type: object\nproperties:\n  port: {type: integer}\n")
        .unwrap()
        .into_owned();
    let sc = glaucus::schema::Schema::from_node(&sc_node);
    let mut doc = glaucus::cst::Document::parse("port: \"8080\"  # listen port\n");
    let changes = glaucus::schema::coerce_to_schema(&mut doc, &sc);
...
fn apply_defaults_via_facade() {
    let sc_node = glaucus::from_str_node(
        "type: object\nproperties:\n  port: {type: integer, default: 8080}\n",
    )
    .unwrap()
    .into_owned();
    let sc = glaucus::schema::Schema::from_node(&sc_node);
    let mut doc = glaucus::cst::Document::parse("name: app  # keep\n");
    let inserted = glaucus::schema::apply_defaults(&mut doc, &sc);
...
fn coerce_array_items_via_facade() {
    ...
    .into_owned();
    let sc = glaucus::schema::Schema::from_node(&sc_node);
    let mut doc = glaucus::cst::Document::parse("ports:\n  - \"80\"  # http\n  - \"443\"\n");
    let changes = glaucus::schema::coerce_to_schema(&mut doc, &sc);
...
fn schema_validation_via_facade() {
    let schema_doc = glaucus::from_str_node(
        "type: object\nrequired: [name]\nproperties:\n  name: {type: string}\n  age: {type: integer}\n",
    )
    .unwrap()
    .into_owned();
    let sc = glaucus::schema::Schema::from_node(&schema_doc);
    let ok = glaucus::from_str_node("name: bob\nage: 5\n")
        .unwrap()
        .into_owned();
    assert!(glaucus::schema::validate(&ok, &sc).is_ok());
    let bad = glaucus::from_str_node("age: notnum\n")
        .unwrap()
        .into_owned();
    let errs = glaucus::schema::validate(&bad, &sc).unwrap_err();
...
fn nested_apply_defaults_via_facade() {
    ...
    let sc = glaucus::schema::Schema::from_node(&sc_node);
    let mut doc = glaucus::cst::Document::parse("server:\n  host: h  # keep\n");
    let _ = glaucus::schema::apply_defaults(&mut doc, &sc);
    assert_eq!(
        doc.to_string(),
        "server:\n  host: h  # keep\n  port: 8080\n"
    );
