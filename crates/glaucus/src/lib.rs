...
//! }
//!
//! let config: Config = glaucus::from_str("name: glaucus\ndebug: true").unwrap();
...
//!
//! ```
//! let docs = glaucus::from_str_multi_node("---\na\n---\nb\n").unwrap();
...
// Re-export glaucus-core public API explicitly (always present).
pub use glaucus_core::{error, limits, parser, scanner, types};

// Re-export glaucus-ast when the `ast` feature is enabled.
#[cfg(feature = "ast")]
pub use glaucus_ast::{self as ast, Mapping, Node, Scalar, Sequence, composer, emitter};

// Re-export glaucus-serde types when the `serde` feature is enabled.
#[cfg(feature = "serde")]
pub use glaucus_serde::{
...
///
/// ```
/// let mut doc = glaucus::cst::Document::parse("version: 0.0.1  # keep\nname: glaucus\n");
/// doc.set("version", "0.0.2").unwrap();
/// assert_eq!(doc.to_string(), "version: 0.0.2  # keep\nname: glaucus\n");
/// assert_eq!(doc.get("name").map(str::trim), Some("glaucus"));
/// ```
#[cfg(feature = "cst")]
pub use glaucus_cst as cst;

/// Span-anchored JSON-Schema validation over the Glaucus `Node` tree.
#[cfg(feature = "schema")]
pub use glaucus_schema as schema;
...
/// Build provenance shared by every crate that reports a version.
///
/// Unconditional: [`version::LONG_VERSION`] is a `concat!` of compile-time
/// literals with no dependencies, so it stays available to library users
/// regardless of which features are active. The rest of the module ---
/// `version::BuildInfo` and its rendering --- is gated behind `cli` inside
/// the module itself, since it needs `serde_json` and `toml`. (Plain code
/// font there, not a link: `BuildInfo` does not exist without `cli`, and this
/// doc comment compiles regardless of that feature.)
pub mod version;
...
///
/// let s: Server = glaucus::from_str("host: localhost\nport: 8080").unwrap();
...
/// # Errors
///
/// Returns an error if `input` is not well-formed YAML, if a configured
/// resource limit is exceeded, or if the document does not match `T`.
#[cfg(feature = "serde")]
pub fn from_str<T: serde::de::DeserializeOwned + 'static>(input: &str) -> glaucus_serde::Result<T> {
    glaucus_serde::from_str(input)
}
...
/// let item: Item = glaucus::from_reader(reader).unwrap();
/// assert_eq!(item.name, "widget");
/// ```
///
/// # Errors
///
/// Returns an error if the reader fails, if the input is not well-formed
/// YAML, if a resource limit is exceeded, or if it does not match `T`.
#[cfg(feature = "serde")]
pub fn from_reader<T: serde::de::DeserializeOwned + 'static>(
    reader: impl Read,
) -> glaucus_serde::Result<T> {
    glaucus_serde::from_reader(reader)
}
...
#[cfg(feature = "serde")]
pub fn to_string<T: serde::Serialize>(value: &T) -> glaucus_serde::Result<String> {
    glaucus_serde::to_string(value)
}
...
) -> glaucus_serde::Result<String> {
    glaucus_serde::to_string_with(value, config)
}
...
/// let c: Config = glaucus::from_str_with("name: app", config).unwrap();
...
pub fn from_str_with<T: serde::de::DeserializeOwned + 'static>(
    input: &str,
    config: error::ParserConfig,
) -> glaucus_serde::Result<T> {
    glaucus_serde::from_str_with(input, config)
}
...
/// ```
/// use serde::Deserialize;
/// use glaucus::error::ParserConfig;
///
/// #[derive(Deserialize)]
/// struct Item { name: String }
///
/// let reader = std::io::Cursor::new(b"name: widget");
/// let item: Item = glaucus::from_reader_with(reader, ParserConfig::default()).unwrap();
/// assert_eq!(item.name, "widget");
/// ```
///
/// # Errors
...
pub fn from_reader_with<T: serde::de::DeserializeOwned + 'static>(
    ...
    glaucus_serde::from_reader_with(reader, config)
}
...
pub fn from_str_multi<T: serde::de::DeserializeOwned + 'static>(
    input: &str,
) -> glaucus_serde::Result<Vec<T>> {
    glaucus_serde::from_str_multi(input)
}
...
/// let docs: Vec<String> = glaucus::from_str_multi_with("---\na\n---\nb\n", config).unwrap();
...
pub fn from_str_multi_with<T: serde::de::DeserializeOwned + 'static>(
    ...
    glaucus_serde::from_str_multi_with(input, config)
}
...
/// let docs: Vec<String> = glaucus::from_reader_multi(reader).unwrap();
/// assert_eq!(docs, vec!["hello", "world"]);
/// ```
///
/// # Errors
...
pub fn from_reader_multi<T: serde::de::DeserializeOwned + 'static>(
    ...
    reader.read_to_string(&mut buf).map_err(|e| {
        glaucus_serde::Error::core(error::Error::spanless(error::ErrorKind::UnexpectedToken {
            expected: "readable input".into(),
            found: format!("I/O error: {e}").into(),
        }))
    })?;
    glaucus_serde::from_str_multi(&buf)
}
...
///
/// let reader = std::io::Cursor::new(b"---\na\n---\nb\n");
/// let docs: Vec<String> = glaucus::from_reader_multi_with(reader, ParserConfig::default()).unwrap();
/// assert_eq!(docs, vec!["a", "b"]);
/// ```
///
/// # Errors
...
pub fn from_reader_multi_with<T: serde::de::DeserializeOwned + 'static>(
    ...
    glaucus_serde::from_str_multi_with(&buf, config)
}
...
/// ```
/// let mut buf = Vec::new();
/// glaucus::to_writer(&mut buf, &vec![1, 2, 3]).unwrap();
/// assert_eq!(std::str::from_utf8(&buf).unwrap(), "- 1\n- 2\n- 3\n");
/// ```
///
/// # Errors
///
/// Returns an error if the writer fails, if `value`'s `Serialize`
/// implementation fails, or if a value cannot be represented in YAML.
#[cfg(feature = "serde")]
pub fn to_writer<T: serde::Serialize, W: std::io::Write>(
    writer: W,
    value: &T,
) -> glaucus_serde::Result<()> {
    glaucus_serde::to_writer(writer, value)
}
...
/// let config = EmitterConfig { explicit_document: true, ..EmitterConfig::default() };
/// let mut buf = Vec::new();
/// glaucus::to_writer_with(&mut buf, &vec![1, 2], &config).unwrap();
...
pub fn to_writer_with<T: serde::Serialize, W: std::io::Write>(
    ...
    glaucus_serde::to_writer_with(writer, value, config)
}
...
///
/// ```
/// let node = glaucus::from_str_node("hello").unwrap();
...
/// # Errors
///
/// Returns an error if `input` is not well-formed YAML or a configured
/// resource limit is exceeded.
#[cfg(feature = "ast")]
pub fn from_str_multi_node(input: &str) -> error::Result<Vec<Node<'_>>> {
    composer::compose_all(input)
}
...
/// let node = glaucus::from_reader_node(reader).unwrap();
/// assert_eq!(node.as_str(), Some("hello"));
/// ```
///
/// # Errors
///
/// Returns an error if the reader fails, if the input is not well-formed
/// YAML, or if a configured resource limit is exceeded.
#[cfg(feature = "ast")]
pub fn from_reader_node<R: Read>(mut reader: R) -> error::Result<Node<'static>> {
    let mut buf = String::new();
    reader.read_to_string(&mut buf).map_err(|e| {
        error::Error::spanless(error::ErrorKind::UnexpectedToken {
            expected: "readable input".into(),
            found: format!("I/O error: {e}").into(),
        })
    })?;
...
/// ```
/// let reader = std::io::Cursor::new(b"---\na\n---\nb\n");
/// let docs = glaucus::from_reader_multi_node(reader).unwrap();
/// assert_eq!(docs.len(), 2);
/// ```
///
/// # Errors
...
/// YAML, or if a configured resource limit is exceeded.
#[cfg(feature = "ast")]
pub fn from_reader_multi_node<R: Read>(mut reader: R) -> error::Result<Vec<Node<'static>>> {
    let mut buf = String::new();
    reader.read_to_string(&mut buf).map_err(|e| {
        error::Error::spanless(error::ErrorKind::UnexpectedToken {
            expected: "readable input".into(),
    ...
    Ok(nodes.into_iter().map(Node::into_owned).collect())
}
...
///
/// ```
/// let node = glaucus::from_str_node("key: value").unwrap();
/// let yaml = glaucus::to_string_node(&node);
/// assert_eq!(yaml, "key: value\n");
/// ```
#[must_use]
#[cfg(feature = "ast")]
pub fn to_string_node(node: &Node<'_>) -> String {
    emitter::emit_to_string(node, &emitter::EmitterConfig::default())
}
...
/// let node = glaucus::from_str_node("key: value").unwrap();
/// let config = EmitterConfig { explicit_document: true, ..EmitterConfig::default() };
/// let yaml = glaucus::to_string_node_with(&node, &config);
/// assert!(yaml.starts_with("---"));
/// ```
#[must_use]
#[cfg(feature = "ast")]
pub fn to_string_node_with(node: &Node<'_>, config: &emitter::EmitterConfig) -> String {
    emitter::emit_to_string(node, config)
}
...
/// let node = glaucus::from_str_node("hello").unwrap();
/// let mut buf = Vec::new();
/// glaucus::to_writer_node(&mut buf, &node).unwrap();
/// assert_eq!(std::str::from_utf8(&buf).unwrap(), "hello\n");
/// ```
#[cfg(feature = "ast")]
pub fn to_writer_node<W: std::io::Write>(mut writer: W, node: &Node<'_>) -> std::io::Result<()> {
...
mod tests {
    ...
    fn serde_from_str_struct() {
        ...
        let c: Config = from_str("name: glaucus\ndebug: true").unwrap();
        assert_eq!(c.name, "glaucus");
        assert!(c.debug);
    }
    ...
    fn serde_value_roundtrip() {
        let value: Value = from_str("name: glaucus\nlist:\n  - a\n  - b").unwrap();
    ...
    fn serde_from_str_with_lenient() {
        ...
        let config = error::ParserConfig {
            strictness: error::Strictness::Lenient,
            ..Default::default()
        };
    ...
    fn serde_from_reader_with() {
        ...
        let item: Item = from_reader_with(reader, error::ParserConfig::default()).unwrap();
    ...
    fn serde_from_str_multi_with_happy_path() {
        let config = error::ParserConfig::default();
    ...
    fn serde_from_str_multi_with_lenient_mode() {
        use glaucus_core::error::Strictness;
        let config = error::ParserConfig {
    ...
    fn serde_from_reader_multi_io_error() {
        let result: glaucus_serde::Result<Vec<String>> = from_reader_multi(FailingReader);
    ...
    fn serde_from_reader_multi_with_io_error() {
        let config = error::ParserConfig::default();
        let result: glaucus_serde::Result<Vec<String>> =
