...
//! Span-anchored JSON-Schema validation with comment-preserving autofix.
//!
//! Moved from the now-retired `glaucus-validate` crate (Task 13 deleted that
//! crate's directory entirely, binary included).
...
pub fn validate_str(data: &str, schema: &str) -> Vec<Diagnostic> {
    let schema_node = match crate::from_str_node(schema) {
        Ok(n) => n,
        Err(e) => {
    ...
    let data_node = match crate::from_str_node(data) {
        Ok(n) => n,
        Err(e) => {
            return vec![Diagnostic {
                line: 0,
    ...
    match crate::schema::validate(&data_node, &sc) {
        Ok(()) => Vec::new(),
        Err(errs) => errs
            .into_iter()
            .map(|e| Diagnostic {
...
/// keys). Both mutations share the same [`cst::Document`](crate::cst::Document)
...
pub fn fix_str(data: &str, schema: &str) -> (String, String) {
    let Ok(schema_node) = crate::from_str_node(schema) else {
        return (
            data.to_string(),
    ...
    let sc = crate::schema::Schema::from_node(&schema_node);
    let mut doc = crate::cst::Document::parse(data);
    let coercions = crate::schema::coerce_to_schema(&mut doc, &sc);
    let insertions = crate::schema::apply_defaults(&mut doc, &sc);
    let report = format!(
...
mod tests {
    ...
    fn validate_str_ok_yields_no_diagnostics() {
        let schema = "type: object\nrequired: [name]\nproperties:\n  name: {type: string}\n";
        assert!(validate_str("name: glaucus\n", schema).is_empty());
    }
