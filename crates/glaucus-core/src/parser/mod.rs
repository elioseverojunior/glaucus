...
// ─── Empty-node predicates ──────────────────────────────────────────
//
// Each answers "does this next-token kind mean the node here is an implicit
// empty scalar?" for one syntactic context.
//
// They are free functions over the peeked kind rather than `&mut self` methods
// so the caller's borrow of the parser ends at the call, and every `matches!`
// is kept on ONE line. The single line is load-bearing, not cosmetic:
// cargo-tarpaulin treats a pattern sitting on its own line as a traceable line,
// but rustc emits no coverage region there, so such a line is reported as
// executed-zero-times no matter what the tests do. Written as a multi-line
// `if let Some(A | B | C) =`, these five predicates plus the `match` in
// `parse_node_indentless` held the file at 530/545 and the workspace at 99.67%
// against a 100% gate — every one of those 15 lines a pattern whose own body
// had a non-zero hit count.

/// True when `kind` terminates the current document, making its body an
/// implicit empty scalar.
const fn ends_document(kind: Option<&TokenKind<'_>>) -> bool {
    use TokenKind::{DocumentEnd, DocumentStart, StreamEnd};
    matches!(kind, Some(DocumentEnd | DocumentStart | StreamEnd))
}

/// True when `kind` closes a block mapping entry, leaving its key or value
/// empty.
const fn ends_block_node(kind: Option<&TokenKind<'_>>) -> bool {
    use TokenKind::{BlockEnd, Key, Value};
    matches!(kind, Some(Key | Value | BlockEnd))
}

/// True when `kind` closes a flow mapping entry, leaving its value empty.
const fn ends_flow_value(kind: Option<&TokenKind<'_>>) -> bool {
    matches!(kind, Some(TokenKind::FlowEntry | TokenKind::FlowMappingEnd))
}

/// True when `kind` closes a flow mapping key, leaving it empty. A `Value`
/// indicator does so too — `{: v}` has an empty key — hence the extra arm over
/// [`ends_flow_value`].
const fn ends_flow_key(kind: Option<&TokenKind<'_>>) -> bool {
    matches!(kind, Some(TokenKind::Value)) || ends_flow_value(kind)
}

/// True when `kind` belongs to the parent context rather than to the indentless
/// node being parsed, making that node an empty scalar. The union of the block
/// and document terminators plus `BlockEntry`, which continues the *parent*
/// sequence.
const fn ends_indentless_node(kind: Option<&TokenKind<'_>>) -> bool {
    ends_document(kind) || ends_block_node(kind) || matches!(kind, Some(TokenKind::BlockEntry))
}
...
impl<'a> Parser<'a> {
    ...
    fn unexpected_token(&mut self, expected: &'static str) -> Result<Error> {
        ...
        let found = self
            .peek_token_kind()?
            .map_or("end-of-input", TokenKind::name)
            .to_string()
            .into();
        Ok(Error::new(
    ...
    fn resolve_tag(&self, handle: &str, suffix: Cow<'a, str>, span: Span) -> Result<TagPair<'a>> {
        if handle.is_empty() {
            // Verbatim tag: !<uri> — suffix is the full URI
            return Ok((Cow::Owned(String::new()), suffix));
        }
        let prefix = self.tag_directives.get(handle).ok_or_else(|| {
            Error::new(
                ErrorKind::UnexpectedToken {
    ...
    /// Reset the tag-directive table to its defaults, then consume any
    /// directive block preceding the next document, skipping bare `...`
    /// markers (a document-end with no document is valid YAML).
    ///
    ...
    fn collect_document_directives(&mut self) -> Result<bool> {
        ...
        Ok(has_any_directive)
    }

    fn parse_document_start(&mut self) -> Result<Option<Event<'a>>> {
        let has_any_directive = self.collect_document_directives()?;
    ...
    fn parse_document_content(&mut self) -> Result<Option<Event<'a>>> {
        if ends_document(self.peek_token_kind()?) {
            // Empty document
    ...
    fn parse_document_end(&mut self) -> Result<Option<Event<'a>>> {
        let (explicit, span) = if matches!(self.peek_token_kind()?, Some(TokenKind::DocumentEnd)) {
            let token = self.require_token()?;
            (true, token.span)
        } else {
            let span = self
    ...
    // One flat `match` from token kind to the event that opens that node, with
    // an arm per YAML node form. The length is the number of forms, not nested
    // logic — every arm is a short, independent mapping and two are selected by
    // guard (`block`, `indentless`). Splitting it would spread one dispatch
    // table across several functions and force the guards to be re-tested in
    // each, so the count is accepted here rather than paid for in indirection.
    ...
    fn parse_node_indentless(&mut self, in_sequence: bool) -> Result<Option<Event<'a>>> {
        if in_sequence {
            ...
            if has_properties {
                ...
                if !ends_indentless_node(self.peek_token_kind()?) {
                    // There's actual content after the anchor/tag — delegate to
                    // the shared dispatch with the properties we already
                    // consumed (block context, indentless allowed).
                    return self.parse_node_inner(true, true, (anchor, tag));
                }
                // Anchor/tag on an empty scalar — the next token belongs to the
                // parent context.
                self.pop_state();
                let span = self
                    .peek_token()?
    ...
    fn parse_block_sequence_entry(
        ...
        match self.peek_token_kind()? {
            Some(TokenKind::BlockEntry) => {
                ...
                if let Some(TokenKind::BlockEntry | TokenKind::BlockEnd) = self.peek_token_kind()? {
                    // Empty entry
    ...
    fn parse_block_mapping_key(&mut self, _first: bool) -> Result<Option<Event<'a>>> {
        ...
        match self.peek_token_kind()? {
            Some(TokenKind::Key) => {
                ...
                if ends_block_node(self.peek_token_kind()?) {
                    // Empty key
    ...
    fn parse_block_mapping_value(&mut self) -> Result<Option<Event<'a>>> {
        if matches!(self.peek_token_kind()?, Some(TokenKind::Value)) {
            let token = self.require_token()?;
    ...
    fn parse_flow_sequence_entry(&mut self, first: bool) -> Result<Option<Event<'a>>> {
        ...
        match self.peek_token_kind()? {
            ...
            Some(TokenKind::Key | TokenKind::Value) => {
    ...
    fn parse_flow_mapping_key(&mut self, first: bool, implicit: bool) -> Result<Option<Event<'a>>> {
        ...
        match self.peek_token_kind()? {
            ...
            Some(TokenKind::Key) => {
                ...
                if ends_flow_key(self.peek_token_kind()?) {
                    // Empty key
    ...
    fn parse_flow_mapping_value(&mut self, implicit: bool) -> Result<Option<Event<'a>>> {
        if matches!(self.peek_token_kind()?, Some(TokenKind::Value)) {
            ...
            if ends_flow_value(self.peek_token_kind()?) {
                // Empty value
...
mod tests {
    ...
    fn event_names(input: &str) -> Vec<&'static str> {
        parse(input).iter().map(EventKind::name).collect()
    }
    ...
    fn plain_scalar() {
        ...
        assert!(events.iter().map(EventKind::name).any(|n| n == "scalar"));
        assert!(matches!(
            &events[2],
    ...
    // ─── Empty nodes: the branches that emit an implicit empty scalar ────────

    #[test]
    fn explicit_document_with_no_content() {
        // `---` immediately followed by `...`: parse_document_content sees a
        // DocumentEnd token where a node was expected and emits the implicit
        // empty scalar rather than descending into parse_node.
        let names = event_names("---\n...\n");
        assert_eq!(
            names,
            [
    ...
    #[test]
    fn anchor_on_empty_block_sequence_entry() {
        // `- &a` with nothing after it: parse_node_indentless(in_sequence=true)
        // consumes the anchor, peeks the next BlockEntry, and resolves the
        // anchor onto an empty scalar -- the `- b` belongs to the PARENT
        // sequence, not to an indentless one nested under the first entry.
        let events = parse("- &a\n- b\n");
        let first = events
            .iter()
            .find(|e| matches!(e, EventKind::Scalar { .. }))
        ...
        assert!(
            matches!(
                first,
                EventKind::Scalar { value, anchor: Some(a), .. }
                    if value.is_empty() && a.as_ref() == "a"
            ),
            "expected empty anchored scalar, got {first:?}"
        );
        // Exactly one sequence: the entries are siblings.
        assert_eq!(
            events
                .iter()
                .filter(|e| matches!(e, EventKind::SequenceStart { .. }))
                .count(),
            1
        );
    }
    ...
    #[test]
    fn anchor_on_nested_block_sequence_entry() {
        // `- &a` followed by an indented `- x`: same entry path as above, but
        // the peek finds real content, so it delegates to the shared dispatch
        // with the already-consumed properties instead of emitting an empty
        // scalar. Two sequences, and the anchor lands on the inner one.
        let events = parse("- &a\n  - x\n");
        assert_eq!(
            events
                .iter()
        ...
        assert!(events.iter().any(|e| matches!(
            e,
            EventKind::SequenceStart { anchor: Some(a), .. } if a.as_ref() == "a"
        )));
    }

    #[test]
    fn block_mapping_empty_key() {
        // `: v` -- a Value token where a key was expected, so the key is the
        // implicit empty scalar.
        let events = parse(": v\n");
    ...
    fn block_mapping_empty_value() {
        // `k:` with nothing after the colon -- BlockEnd follows the Value
        // token, so the value is the implicit empty scalar.
        let events = parse("k:\n");
        let scalars: Vec<_> = events
            .iter()
            .filter_map(|e| match e {
    ...
    fn flow_mapping_empty_key() {
        // `{: v}` -- the flow-mapping analogue of `: v`.
        let events = parse("{: v}\n");
        let scalars: Vec<_> = events
            .iter()
            .filter_map(|e| match e {
    ...
    // The YAML `{k: }` has the shape of a format placeholder; it is data.
    #[allow(clippy::literal_string_with_formatting_args)]
    #[test]
    fn flow_mapping_empty_value() {
        // `{k: }` -- FlowMappingEnd follows the Value token, so the value is
        // the implicit empty scalar.
        let events = parse("{k: }\n");
        let scalars: Vec<_> = events
            .iter()
            .filter_map(|e| match e {
