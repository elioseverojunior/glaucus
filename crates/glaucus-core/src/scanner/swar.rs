...
// These helpers are single arithmetic expressions — a multiply, a mask, a
// `trailing_zeros`. They exist to be folded into the caller's 8-bytes-at-a-time
// loop, where a preserved call would cost more than the work it wraps and would
// defeat the point of scanning a word at a time. `#[inline]` only advises, so
// the lint is answered once here rather than per function.
...
#[inline(always)]
const fn match_offset(mask: u64) -> usize {
...
mod tests {
    ...
    fn has_value_detects_colon() {
        let word = u64::from_le_bytes(*b"hello:12");
