...
/// Puts a column on the same scale as [`Scanner::indent`].
///
/// Indent levels are signed because `-1` is the "no indent established"
/// sentinel, while columns are unsigned. The document-size limit keeps columns
/// far below `i32::MAX`, so the saturation never fires in practice; it is here
/// so that no input can wrap a column into a negative value, which would be
/// read as the sentinel and silently reset the indent stack.
#[inline]
fn indent_of(column: u32) -> i32 {
    i32::try_from(column).unwrap_or(i32::MAX)
}

/// Puts an indent level back on the column scale, mapping the `-1` sentinel
/// (and any other negative level) to column 0.
#[inline]
fn column_of(indent: i32) -> u32 {
    u32::try_from(indent.max(0)).unwrap_or(0)
}

/// Widens a byte count to a column delta, saturating instead of truncating on
/// 64-bit targets where `usize` is wider than the column counter.
#[inline]
fn column_span(n: usize) -> u32 {
    u32::try_from(n).unwrap_or(u32::MAX)
}

/// Append the breaks accumulated between two block-scalar content lines,
/// applying the folding rules of YAML 1.2.2 §8.2.1.
///
/// Literal scalars keep every break. Folded scalars fold a lone break between
/// two normally-indented lines into a space, but preserve it whenever either
/// side is more-indented. Where blank lines intervene, the first break is the
/// previous line's ending and is dropped; the blank ones survive.
fn flush_block_scalar_breaks(
    content: &mut String,
    trailing_breaks: &str,
    style: ScalarStyle,
    prev_more_indented: bool,
    current_more_indented: bool,
) {
    if style != ScalarStyle::Folded || content.is_empty() {
        content.push_str(trailing_breaks);
    } else if trailing_breaks == "\n" {
...
//
// The state flags below are genuinely independent latches in a hand-written
// tokenizer — each is set and cleared by a different YAML production, and they
// are never read as a group. Packing them into a bitflags newtype to satisfy a
// count would add indirection at every read without making any state clearer.
...
impl<'a> Scanner<'a> {
    ...
    #[must_use]
    pub const fn preserve_trivia(&self) -> bool {
    ...
    fn next_with_trivia(&mut self) -> Option<Result<Token<'a>>> {
        ...
        let sig_offset = if has_sig {
            self.tokens
                .front()
                .map_or(usize::MAX, |t| t.span.start.offset)
        } else {
            usize::MAX
        };
    ...
    const fn span_from(&self, start: Position) -> Span {
        Span {
            start,
            end: self.pos(),
        }
    ...
    const fn check_document_size(&self) -> Result<()> {
    ...
    fn fetch_next_token(&mut self) -> Result<bool> {
        ...
        self.unroll_indent(indent_of(self.column));
        ...
        if self.value_property_min_column >= 0 {
            if (b == b'&' || b == b'!') && self.flow_level == 0 {
                if indent_of(self.column) < self.value_property_min_column {
        ...
        loop {
            ...
            while let Some(b) = self.peek() {
                if b == b' ' {
                    ...
                    if at_line_start
                        && self.flow_level == 0
                        && indent_of(self.column) <= self.indent
                    {
                        return Err(Error::new(
                            ErrorKind::UnexpectedToken {
                    ...
                    if at_line_start && self.flow_level > 0 && indent_of(self.column) <= self.indent
                    {
    ...
    // One of the sixteen `fetch_*` dispatch arms in `fetch_next_token`, which
    // all share `Result<bool>` and are returned straight out of the match. This
    // production has nothing to reject today; narrowing just this one to `bool`
    // would force an `Ok(..)` wrap at its arm and flip back the moment a
    // validation rule lands on it.
    ...
    fn fetch_block_entry(&mut self) -> Result<bool> {
        ...
        if self.flow_level == 0 {
            ...
            self.roll_indent(indent_of(self.column), TokenKind::BlockSequenceStart, None);
        }
        self.remove_simple_key();
        self.allow_simple_key = true;
    ...
    fn fetch_key(&mut self) -> Result<bool> {
        ...
        if self.flow_level == 0 {
            ...
            self.roll_indent(indent_of(self.column), TokenKind::BlockMappingStart, None);
        }
        self.remove_simple_key();
    ...
    fn percent_decode(s: &str) -> Cow<'_, str> {
        ...
        while i < bytes.len() {
            if bytes[i] == b'%' && i + 2 < bytes.len() {
                // Narrow each nibble to `u8` up front: `to_digit(16)` yields
                // 0..=15, so both conversions succeed and the packed byte
                // cannot overflow — which lets the pack itself stay cast-free.
    ...
    /// Consume a block scalar header: the chomping and indentation indicators,
    /// any trailing spaces and comment, and the line break that ends it.
    ///
    /// Entered just past the `|` or `>`, and left at the start of the first
    /// content line. Returns the chomping mode (-1 strip, 0 clip, 1 keep) and
    /// the explicit indentation indicator if one was given.
    fn scan_block_scalar_header(&mut self, start: Position) -> Result<(i8, Option<u32>)> {
        let mut chomping = 0i8;
        let mut explicit_indent: Option<u32> = None;

        // The two indicators may appear in either order, so take up to two.
        for _ in 0..2 {
            if let Some(b) = self.peek() {
                if b == b'+' {
                    chomping = 1;
                    ...
                    explicit_indent = Some(u32::from(b - b'0'));
                    self.advance();
                }
            }
        }
    ...
    /// Resolve the indentation a block scalar's content is measured against.
    ///
    /// An explicit indicator is relative to the enclosing indent. Otherwise the
    /// first content line sets it, by look-ahead that does not move the cursor.
    fn detect_block_scalar_indent(&self, explicit_indent: Option<u32>) -> Result<u32> {
        if let Some(n) = explicit_indent {
            return Ok(column_of(self.indent) + n);
        }
        ...
        Ok(auto_indent.max(column_of(self.indent + 1)))
    }
    ...
    fn fetch_block_scalar(&mut self, style: ScalarStyle) -> Result<bool> {
        ...
        let (chomping, explicit_indent) = self.scan_block_scalar_header(start)?;
        let block_indent = self.detect_block_scalar_indent(explicit_indent)?;
        ...
        loop {
            ...
            // We have a content line — flush the breaks that preceded it.
            flush_block_scalar_breaks(
                &mut content,
                &trailing_breaks,
                style,
                prev_more_indented,
                current_more_indented,
            );
    ...
    /// Decode the escape sequence introduced by a `\` in a double-quoted
    /// scalar, appending its expansion to `content`.
    ///
    /// Entered with the cursor on the backslash and left just past the
    /// sequence. `start` is the opening quote, used only for error spans.
    ///
    /// The single-character escapes (YAML 1.2.2 §5.7) are a plain byte-to-char
    /// table; only the hex forms, the escaped line break and the rejection path
    /// need arms of their own.
    fn scan_double_quoted_escape(&mut self, content: &mut String, start: Position) -> Result<()> {
        self.advance(); // past the backslash
        let Some(b) = self.peek() else {
            return Err(Error::new(ErrorKind::UnexpectedEof, self.span_from(start)));
        };

        // Hex escapes carry a payload of 2, 4 or 8 digits.
        if let Some(digits) = match b {
            b'x' => Some(2),
            b'u' => Some(4),
            b'U' => Some(8),
            _ => None,
            ...
            self.advance();
            let ch = self.scan_hex_escape(digits, start)?;
            content.push(ch);
            return Ok(());
        }

        // An escaped line break folds away entirely, along with the
        ...
        let ch = match b {
            ...
            _ => {
                return Err(Error::new(
                    ErrorKind::InvalidEscape(char::from(b)),
                    self.span_from(start),
                ));
            }
        };
        ...
        loop {
            match self.peek() {
                ...
                Some(b'\\') => {
                    content.push_str(&trailing_ws);
                    trailing_ws.clear();
                    self.scan_double_quoted_escape(&mut content, start)?;
                }
                Some(b) if chars::is_break(b) => {
                    ...
                    while let Some(b2) = self.peek() {
                        if b2 == b' ' {
                            ...
                            if self.flow_level == 0 && indent_of(self.column) <= self.indent {
                                return Err(Error::new(
                                    ErrorKind::UnexpectedToken {
                                        expected: "spaces for indentation".into(),
                                        found: "tab character".into(),
    ...
    // A single scan loop whose locals are the lazy-borrow optimisation: the
    // scalar stays a borrowed `content_start..value_end` span until a line
    // break forces folding, at which point `content`, `whitespace`,
    // `leading_break`, `trailing_breaks` and `all_borrowed` take over together.
    // Those six move as one state; carving the loop into helpers would mean
    // threading all of them as `&mut` parameters, or boxing them into a struct
    // that exists only to be unpacked again — more surface, same logic, and a
    // real risk of desyncing the borrowed and owned paths.
    ...
    fn fetch_plain_scalar(&mut self) -> Result<bool> {
        ...
        loop {
            ...
            let content_empty = content
                .as_ref()
                .map_or(value_end == content_start, String::is_empty);
        ...
        if content_empty {
            let next = self
                .peek()
                .map_or_else(|| "EOF".to_string(), |b| char::from(b).to_string());
...
mod tests {
    ...
    fn block_mapping() {
        ...
        let names: Vec<&str> = tokens.iter().map(TokenKind::name).collect();
    ...
    fn document_size_limit() {
        ...
        let has_error = result.iter().any(std::result::Result::is_err);
