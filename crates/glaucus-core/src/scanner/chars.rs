...
// Every predicate here compiles to one table index plus a test — smaller than
// the call sequence that would set it up. `#[inline]` only advises, and these
// sit in the scanner's innermost byte loop where a preserved call would cost
// more than the body. The attribute is the point of the module, so the lint is
// answered once here rather than per function.
...
const fn build_char_flags() -> [u8; 256] {
    ...
    // Walk the byte domain in `u8` and break on the last value rather than
    // counting in a wider type and narrowing each step: `0..256` in a `u16`
    // needs an `i as u8` that is provably in range but that the compiler (and
    // `cast_possible_truncation`) cannot see through. `b as usize` below only
    // widens.
    let mut b = 0u8;
    loop {
        let mut flags = 0u8;
...
#[must_use]
#[inline(always)]
pub const fn is(b: u8, flags: u8) -> bool {
...
#[must_use]
#[inline(always)]
pub const fn is_break(b: u8) -> bool {
    is(b, FLAG_BREAK)
}
...
#[must_use]
#[inline(always)]
pub const fn is_blank_or_break(b: u8) -> bool {
...
#[must_use]
#[inline(always)]
pub const fn is_flow(b: u8) -> bool {
    is(b, FLAG_FLOW)
}
...
#[must_use]
#[inline(always)]
pub const fn is_indicator(b: u8) -> bool {
    is(b, FLAG_INDICATOR)
}
...
#[must_use]
#[inline(always)]
pub const fn is_anchor_char(b: u8) -> bool {
    is(b, FLAG_ANCHOR)
}
